...
/**
 * Option utils for JedisOpOptions table source sink.
 */
public final class JedisOpOptions implements Serializable {
		...
		switch (dataType) {
			...
			case SORTED_SET:
				return new RedisSortedSetConsumer(
						...
						logicalTypes.get(1) instanceof ArrayType);
			default:
				throw notSupportDataTypeException();
		}
	}

	private IllegalArgumentException notSupportDataTypeException() {
		return new IllegalArgumentException("Not support " + dataType + " now!");
	}
	...
	public boolean equals(Object o) {
		...
		JedisOpOptions that = (JedisOpOptions) o;
		return ignoreError == that.ignoreError &&
				batchSize == that.batchSize &&
				lingerMs == that.lingerMs &&
				dataType == that.dataType &&
				Objects.equals(keyPattern, that.keyPattern) &&
				Arrays.deepEquals(pubsubPatterns, that.pubsubPatterns) &&
				Objects.equals(channel, that.channel);
	}

	@Override
	public int hashCode() {
		int result = Objects.hash(dataType, keyPattern, ignoreError, batchSize, lingerMs, channel);
		result = 31 * result + Arrays.hashCode(pubsubPatterns);
		return result;
	}
}
